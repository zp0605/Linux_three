...
window.FlexPaperViewer = window.$f = function() {
	...
	window.FlexPaperViewer_Instance = flashembed(arguments[1], {
			...
			PagesNumber: config.PagesNumber,
...
(function() {
	...
	var f = extend(window.flashembed, {
		...
		getHTML: function(opts, conf) {
			...
			var html = '<object wmode="transparent" width="' + opts.width + 
				'" height="' + opts.height + 
				'" id="' + opts.id + 
				'" name="' + opts.id + '"';
			...
			/******* nested PARAM tags *******/
			if (opts.w3c || IE) {
				html += '<param name="movie" value="' +opts.src+ '" /><param name="wmode" value="transparent">'; 	
			} 
