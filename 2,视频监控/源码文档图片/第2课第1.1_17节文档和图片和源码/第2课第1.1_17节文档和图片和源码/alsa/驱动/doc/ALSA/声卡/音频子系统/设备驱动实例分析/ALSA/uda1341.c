...
struct uda1341 {
	int (*write) (struct l3_client *uda1341, unsigned short reg, unsigned short val);  /* 写 */
	int (*read) (struct l3_client *uda1341, unsigned short reg); /* 读 */
...
/* 用于获取该control的详细信息，该函数必须填充第二个参数 */
...
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .info = snd_uda1341_info_single, \  /* 混音器设备 */
...
static struct snd_kcontrol_new snd_uda1341_controls[] = {
	...
	/* 
         { .iface = SNDRV_CTL_ELEM_IFACE_MIXER, 
            .name = "Master Playback Switch", // 混音器设备 
            .info = snd_uda1341_info_single, \  
            .get = snd_uda1341_get_single, 
            .put = snd_uda1341_put_single, \
            .private_value = CMD_MUTE | (data0_2 << 5) | (2 << 9) | (1 << 12) | (1 << 18) 
},
	  */
...
static struct snd_device_ops ops = {  /* 声卡设备操作函数 */
...
int __init snd_chip_uda1341_mixer_new(struct snd_card *card, struct l3_client **clntp)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(snd_uda1341_controls); idx++) {
		if ((err = snd_ctl_add(card, snd_ctl_new1(&snd_uda1341_controls[idx], NULL))) < 0)   /*  创建一个control实例并添加 声卡中*/
			return err;
	}

	if ((err = snd_device_new(card, SNDRV_DEV_CODEC, NULL, &ops)) < 0)  /* 创建编解码设备 */
		return err;

	strcpy(card->mixername, "UDA1341TS Mixer");  /* card->mixername=UDA1341TS Mixer*/
