...
struct snd_soc_pcm_stream {  /* SoC PCM流信息描述结构体 */
	char *stream_name;
...
/* SoC Audio Codec */
struct snd_soc_codec { /* SoC音频编解码器 */
	char *name;  /* 名字 */
...
/* codec device */
struct snd_soc_codec_device { /* 编解码设备操作函数 */
...
struct snd_soc_platform { /* SoC平台接口 */
	char *name;
	struct list_head list;
	...
	/* platform stream ops */
	struct snd_pcm_ops *pcm_ops; /* pcm操作函数 */
};
...
/* SoC card */
struct snd_soc_card {  /* SoC卡 */
	char *name;  /* 名字 */
	struct device *dev; /* 内嵌的设备模型的设备 */
  
	struct list_head list;  /* 用于形成链表 */

	int instantiated; /* 实例化标志位 */
	...
	/* CPU <--> Codec DAI links  */
	struct snd_soc_dai_link *dai_link;  /* 绑定ASoC Codec驱动和CPU端的平台驱动数据结构 */
	int num_links;   /* CPU和codec绑定的数目 */

	struct snd_soc_device *socdev; /* SoC设备 */

	struct snd_soc_codec *codec;/* SoC编解码设备 */

	struct snd_soc_platform *platform; /* CPU段的平台驱动 */
	struct delayed_work delayed_work;  /* 延时工作 */
...
struct snd_soc_pcm_runtime {  /* 运行时通道数据 */
	struct snd_soc_dai_link *dai;
	struct snd_soc_device *socdev;
};

/* mixer control */
struct soc_mixer_control { /* SoC混音器控制 */
	int min, max;
