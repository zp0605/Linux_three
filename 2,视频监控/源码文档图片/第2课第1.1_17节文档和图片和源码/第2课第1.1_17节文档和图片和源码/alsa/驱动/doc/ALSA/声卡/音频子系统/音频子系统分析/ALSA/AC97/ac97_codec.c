...
struct ac97_codec_id { /* AC97  编解码器 ID*/
	unsigned int id;
	unsigned int mask;
	const char *name;
...
int snd_ac97_update(struct snd_ac97 *ac97, unsigned short reg, unsigned short value)
{
	...
	if (!snd_ac97_valid_reg(ac97, reg))
		return -EINVAL;
	mutex_lock(&ac97->reg_mutex);
	change = ac97->regs[reg] != value;  /* 如果寄存器中的值和给定的值不等则写寄存器 */
	if (change) {
		ac97->regs[reg] = value;
		ac97->bus->ops->write(ac97, reg, value);
	}
				...
				unsigned short mask, unsigned short value)
{
	...
	old = snd_ac97_read_cache(ac97, reg);  /* 读寄存器缓存 */
...
static int snd_ac97_ad18xx_update_pcm_bits(struct snd_ac97 *ac97, int codec, unsigned short mask, unsigned short value)
{
	...
	if (change) {
		...
		ac97->spec.ad18xx.pcmreg[codec] = new;  /* 更新寄存器缓存 */
		/* select single codec */
