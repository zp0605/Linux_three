...
/* 
  AC97的全称为Audio CODEC 97，ALSA AC97编解码层被定义的很好，利用它，驱动工程师只需编写少量底层的控制函数
  */
...
struct snd_ac97_build_ops { /* 构造操作函数 */
...
struct snd_ac97_bus_ops { /* AC97总线操作函数 */
	void (*reset) (struct snd_ac97 *ac97);  /* 复位 */
	void (*warm_reset)(struct snd_ac97 *ac97);  /* 警告复位 */
	void (*write) (struct snd_ac97 *ac97, unsigned short reg, unsigned short val); /* 完成实际硬件的写*/
	unsigned short (*read) (struct snd_ac97 *ac97, unsigned short reg); /* 完成实际硬件的 读 */
	void (*wait) (struct snd_ac97 *ac97);  /* 等待 */
	void (*init) (struct snd_ac97 *ac97); /* 初始化 */
};

struct snd_ac97_bus {  /* AC97总线 */
	/* -- lowlevel (hardware) driver specific -- */
	struct snd_ac97_bus_ops *ops;  /* 总线操作函数 */
	void *private_data; /* 私有数据 */
	void (*private_free) (struct snd_ac97_bus *bus); /* 释放私有数据 */
	/* --- */
	struct snd_card *card; /* 声卡 */
	unsigned short num;	/* bus number */ /* 总线数量 */
	unsigned short no_vra: 1, /* bridge doesn't support VRA */
		       dra: 1,	/* bridge supports double rate */
		       isdin: 1;/* independent SDIN */
	unsigned int clock;	/* AC'97 base clock (usually 48000Hz) */ /* AC97基准时钟，通常是48000Hz */
	spinlock_t bus_lock;	/* used mainly for slot allocation */  
	unsigned short used_slots[2][4]; /* actually used PCM slots */ /* 实际使用的PCM插槽 */
	unsigned short pcms_count; /* count of PCMs */
	struct ac97_pcm *pcms;  /* pcm设备 */
	struct snd_ac97 *codec[4];  /* AC97设备 */
	struct snd_info_entry *proc;
};

/* static resolution table */
struct snd_ac97_res_table { /* AC97 分辨率表 */
	unsigned short reg;	/* register */
	unsigned short bits;	/* resolution bitmask */
};

struct snd_ac97_template { /* AC97模板 */
	void *private_data;
...
struct snd_ac97 { /* AC97设备 */
	/* -- lowlevel (hardware) driver specific -- */
	struct snd_ac97_build_ops * build_ops;
	void *private_data;
	void (*private_free) (struct snd_ac97 *ac97);
	/* --- */
	struct snd_ac97_bus *bus; /* 总线 */
	struct pci_dev *pci;	/* assigned PCI device - used for quirks */
	struct snd_info_entry *proc;
	...
	unsigned short num;	/* number of codec: 0 = primary, 1 = secondary */ /* 编解码器数量，0为第1个 */
	unsigned short addr;	/* physical address of codec [0-3] */ /* 编解码器的地址 */
	unsigned int id;	/* identification of codec */  /* 编解码器ID */
	...
	unsigned int rates[6];	/* see AC97_RATES_* defines *//* 速率，查看宏 AC97_RATES_* */
	unsigned int spdif_status;
	unsigned short regs[0x80]; /* register cache */ /*寄存器缓存 */
	DECLARE_BITMAP(reg_accessed, 0x80); /* bit flags */
	union {			/* vendor specific code */
		struct {
			unsigned short unchained[3];	/*0 = C34, 1 = C79, 2 = C69*/
			unsigned short chained[3];	/* 0 = C34, 1 = C79, 2 = C69*/
			unsigned short id[3];		/* codec IDs (lower 16-bit word)*/ /* 编解码器 */
			unsigned short pcmreg[3];	/*PCM registers*/  /* PCM寄存器 */
	...
	unsigned int power_up;	/* power states */ /* 电源标志位 */
