...
struct audio_stream {
	char *id;		/* identification string */  /* 识别字符串 */
	int stream_id;		/* numeric identification */	 /* 数量识别 */
	dma_device_t dma_dev;	/* device identifier for DMA */

	unsigned char dmach;		/* dma channel identification */ /* DMA通道 */
	unsigned int active:1;	/* we are using this stream for transfer now */ /* 标识这个流是否正在传输 */
	int period;		/* current transfer period */  /* 当前传输周期 */
	int periods;		/* current count of periods registerd in the DMA engine */ /* 当前计数 */
	int tx_spin;		/* are we recoding - flag used to do DMA trans. for sync */  
	unsigned int old_offset;
	spinlock_t dma_lock;	/* for locking in DMA operations (see dma-sa1100.c in the kernel) */
	struct snd_pcm_substream *stream; /* PCM子流 */
};
...
struct s3c24xx_uda1341 {
	...
	long samplerate;         /* 采样率 */
	struct audio_stream s[2];	/* playback & capture */ /* PCM流---播放流和录音流 */
};


static unsigned int rates[] = {  /* 采样率 */
	8000,  10666, 10985, 14647,
	16000, 21970, 22050, 24000,
	29400, 32000, 44100, 48000,
};

static struct snd_pcm_hw_constraint_list hw_constraints_rates = {  /* 硬件约束列表 */
...
static void s3c24xx_uda1341_set_audio_clock(long val)
{
	switch (val) {
...
static void s3c24xx_uda1341_set_samplerate(struct sa11xx_uda1341 *sa11xx_uda1341, long rate)
{
	int clk_div = 0;
	int clk=0;
...
static void s3c24xx_uda1341_audio_init(struct sa11xx_uda1341 *sa11xx_uda1341)
{
	unsigned long flags;

	/* Setup DMA stuff */
	s3c24xx_uda1341->s[SNDRV_PCM_STREAM_PLAYBACK].id = "UDA1341 out";
	s3c24xx_uda1341->s[SNDRV_PCM_STREAM_PLAYBACK].stream_id = SNDRV_PCM_STREAM_PLAYBACK;
	s3c24xx_uda1341->s[SNDRV_PCM_STREAM_PLAYBACK].dma_dev = DMA_Ser4SSPWr;

	s3c24xx_uda1341->s[SNDRV_PCM_STREAM_CAPTURE].id = "UDA1341 in";
	s3c24xx_uda1341->s[SNDRV_PCM_STREAM_CAPTURE].stream_id = SNDRV_PCM_STREAM_CAPTURE;
...
static void s3c24xx_uda1341_audio_shutdown(struct sa11xx_uda1341 *sa11xx_uda1341)
{
	/* mute on */
	set_sa11xx_uda1341_egpio(IPAQ_EGPIO_QMUTE);
...
static struct s3c2410_dma_client s3c2410_dma_out= {
	.name = "I2SSDO",
};

static struct s3c2410_dma_client s3c2410_dma_in = {
	.name = "I2SSDI",
};
...
static int audio_dma_request(struct audio_stream *s, void (*callback)(void *, int))
{
	...
	ret = s3c2410_dma_request(&s->dmach, &s3c2410_dma_out, s->dma_dev);
	if (ret < 0) {
...
static int __init snd_card_s3c24xx_uda1341_pcm(struct sa11xx_uda1341 *sa11xx_uda1341, int device)
{
	struct snd_pcm *pcm;
	int err;
	...
	s3c24xx_uda1341_audio_init(sa11xx_uda1341);
...
static int snd_s3c24xx_uda1341_suspend(struct platform_device *devptr,
				      pm_message_t state)
{
...
static int snd_s3c24xx_uda1341_resume(struct platform_device *devptr)
{
	struct snd_card *card = platform_get_drvdata(devptr);
	struct sa11xx_uda1341 *chip = card->private_data;
...
#ifdef HH_VERSIONe	
	s3c24xx_dma_wakeup(chip->s[SNDRV_PCM_STREAM_PLAYBACK].dmach);
	s3c24xx_dma_wakeup(chip->s[SNDRV_PCM_STREAM_CAPTURE].dmach);
#else
	//FIXME
#endif
...
static int __devinit s3c24xx_uda1341_probe(struct platform_device *devptr)
{
	int err;
	...
	card = snd_card_new(-1, id, THIS_MODULE, sizeof(struct s3c24xx_uda1341));  /* 创建一个声卡 ， 分配的struct s3c24xx_uda1341结构体的内存空间的首地
	                                                                                                                            址保存在card->private_data */
	if (card == NULL)
		return -ENOMEM;
	...
	card->private_free = snd_s3c24xx_uda1341_free; /* 用于释放 struct s3c24xx_uda1341结构体*/
	chip->card = card;
	chip->samplerate = AUDIO_RATE_DEFAULT;  /* 采样率为44100Hz */
	...
	// PCM
	if ((err = snd_card_s3c24xx_uda1341_pcm(chip, 0)) < 0)
		goto nodev;
...
static int __devexit s3c24xx_uda1341_remove(struct platform_device *devptr)
{
...
static struct platform_driver s3c24xx_uda1341_driver = {
	...
	.remove		= __devexit_p(s3c24xx_uda1341_remove),
#ifdef CONFIG_PM
	.suspend	= snd_s3c24xx_uda1341_suspend,
	.resume		= snd_s3c24xx_uda1341_resume,
#endif
	.driver		= {
...
{
	return platform_driver_unregister(&s3c24xx_uda1341_driver);
}
...
MODULE_VERSION("v1.0");       
MODULE_DESCRIPTION("A audio driver"); 